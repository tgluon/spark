...
private[spark] class SparkSubmit extends Logging {
   ...
   * tailrec： 这个注解是与尾递归优化有关的
   */
  @tailrec
  private def submit(args: SparkSubmitArguments, uninitLog: Boolean): Unit = {

    def doRunMain(): Unit = {
      if (args.proxyUser != null) { // 提交用户不等于null
  ...
  : (Seq[String], Seq[String], SparkConf, String) = {
    ...
    // 判断集群模式
    val clusterManager: Int = args.master match { // --master yarn \
      case "yarn" => YARN
    ...
    // 设置部署模式，默认是client模式
    var deployMode: Int = args.deployMode match { // --deploy-mode cluster
    ...
    if (clusterManager == YARN) {
      ...
      // 判断是否找到YarnClusterApplication类加载器
    ...
    // 如果为NULL，请更新args.deployMode。 稍后将其作为Spark属性传递。
