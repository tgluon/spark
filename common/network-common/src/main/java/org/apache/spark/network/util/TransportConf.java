...
public class TransportConf {
    ...
    public TransportConf(String module, ConfigProvider conf) {
        ...
        // spark.shuffle.io.preferDirectBufs:启用堆外内存，可以避免shuffle过程的频繁gc，如果堆外内存非常紧张，则可以考虑关闭这个选项
        SPARK_NETWORK_IO_PREFERDIRECTBUFS_KEY = getConfKey("io.preferDirectBufs");
        SPARK_NETWORK_IO_CONNECTIONTIMEOUT_KEY = getConfKey("io.connectionTimeout");
        SPARK_NETWORK_IO_BACKLOG_KEY = getConfKey("io.backLog");
        // spark.shuffle.io.numConnectionsPerPeer:机器之间的可以重用的网络连接，主要用于在大型集群中减小网络连接的建立开销，如果一个集群的机器并不多，可以考虑增加这个值
        SPARK_NETWORK_IO_NUMCONNECTIONSPERPEER_KEY = getConfKey("io.numConnectionsPerPeer");
        SPARK_NETWORK_IO_SERVERTHREADS_KEY = getConfKey("io.serverThreads");// spark.{driver|executor}.rpc.io.serverThreads:服务线程数
        ...
        //spark.shuffle.io.maxRetries:shuffle read task从shuffle write task所在节点拉取属于自己的数据时，如果因为网络异常导致拉取失败，是会自动进行重试的。
        ...
        // spark.shuffle.io.retryWait:每次重试拉取数据的等待间隔，默认是5s，建议加大时长，理由同上，保证shuffle操作的稳定性
     ...
     * 启用堆外内存，可以避免shuffle过程的频繁gc，默认是true
     */
     ...
     * 是否使用单独的 EventLoopGroup 来处理 ChunkFetchRequest 消息，由配置 `spark.shuffle.server.chunkFetchHandlerThreadsPercent` 设置与否。
     */
     ...
     * 是否使用旧的FetchProtocol，当版本升级的时候，外部shuffle service 需设置该协议为true，默认是false
     */
